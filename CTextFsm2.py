...
class MyIpAddress(object):
    ...
    @staticmethod
    def is_ipinnetw(sip, snet, isstrict=False):
        """ ip-addressがnetwork-addressに含まれるか否かを検査
        ...
        ## NET_IP to IPv4
        try:
            net_ip = ipaddress.IPv4Address(unicode(snet_ip, 'utf-8'))
            inet = int(net_ip)
        ...
        ## NET_WILD to IPv4
        try:
            net_wild = ipaddress.IPv4Address(unicode(snet_wild, 'utf-8'))
            iwildmask = int(net_wild)
        except (ipaddress.NetmaskValueError, ValueError) as e:
            MyIpAddress.errprint(pgm, caller, 'exception catch:' + str(e))
            return False
        
        isubnetmask = ~iwildmask
        
        if (iip & isubnetmask) == (inet & isubnetmask):
            return True
        else:
            return False
        pass
            ...
            for line in logf:
                ...
                if re.match(r'^!$', line) and is_interface is True:
                    if ipaddress is not '' and ipmask is not '' and \
                        ...
                        _ipaddress = ipaddress + '/' + ipmask
                        ipmask = MyIpAddress.ipwild2ip(ipaddress, ipmask)
                    rtn.append([interface, description, _ipaddress, ipmask, shutdown, aclg, acldir])
                    is_interface = False
        return rtn
...
class CTextFsm(object):
    ...
    def _ismatch_aclstd(self, testptn, a_aclobj):
        ...
        is_src = MyIpAddress.is_ipinnetw(test_src, a_aclobj_src, self.ipnet_strict)
    ...
    def _ismatch_aclext(self, testptn, a_aclobj):
        ...
        pass
        if a_aclobj_dst != '':
            is_dst = MyIpAddress.is_ipinnetw(test_dst, a_aclobj_dst, self.ipnet_strict)
        else:
...
#                dst = MyIpAddress.ipwild2ip(dstip, dstmask)
    ...
    def ut_is_ipinnetw(self, expect, ip, net):
        ...
        """
        try:
            ret = MyIpAddress.is_ipinnetw(ip, net, self.ipnet_strict)
        except ValueError as e:
            ret = False
            print e
        okng = 'UTOK' if ret == expect else 'UTNG'
