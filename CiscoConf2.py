...
class CiscoCommon(CTextFsm):
    ...
    def main(self):
        ...
        if cmd == '':
            ...
            is_address = MyIpAddress.is_ipaddress(address)
            is_wildmask = MyIpAddress.is_ipaddress(wildmask)
            if not (is_address and is_wildmask):
                self.errprint('netwild', '', ('address:%s wildmask:%s' % (is_address, is_wildmask)))
                exit(1)
            print address, wildmask, '->', MyIpAddress.ipwild2ip(address, wildmask)
        elif cmd == 'ipinnet':
            if argc < 4:
            ...
            n = net.split('/')
            is_ip = MyIpAddress.is_ipaddress(ip)
            is_net = MyIpAddress.is_ipaddress(n[0])
            ...
            if int(n[1]) < 0 or int(n[1]) > 32:
                ...
                exit(1)
            print ip, 'in', net, '->', MyIpAddress.is_ipinnet(ip, net)
        elif cmd == 'ipinwild':
            if argc < 5:
            ...
            is_ip = MyIpAddress.is_ipaddress(ip)
            is_net = MyIpAddress.is_ipaddress(net)
            is_wild = MyIpAddress.is_ipaddress(wild)
            net2 = net + '/' + wild
            if not (is_ip and is_net and is_wild):
                self.errprint('ipinwild', '', ('ip:%s network:%s wildmask:%s' % (is_ip, is_net, is_wild)))
                exit(1)
            print ip, 'in', net, wild, '(' + net2 + ')', '->', MyIpAddress.is_ipinnetw(ip, net2)
        elif cmd == 'ipinf':
            if argc < 3:
